...
class 组织介绍(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, zid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.zid = str(zid)
    ...
    def test_body(self):
        ...
        # 1.咨询，2.人物，3.评论
        self.zid = get_excel("zid", self.No, interfaceNo)
        self.data = {
            "id" : self.zid,
            "v": "3.11.0",
            "system": "5",
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取组织介绍"))
