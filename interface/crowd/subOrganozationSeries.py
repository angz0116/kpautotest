...
interfaceNo = "subOrganozationSeries"
name = "组织主页下属机构分类"
...
class 组织主页下属机构分类(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, sid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.sid = str(sid)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取组织主页下属机构分类"))
