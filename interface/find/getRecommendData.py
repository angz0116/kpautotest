...
class 推荐精选动态换一换(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, clicknum, type, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.clicknum = str(clicknum)
        self.type = str(type)
    ...
    def test_body(self):
        ...
        # 第几次点击（下拉刷新时，重制为1）
        self.clicknum = get_excel("clicknum", self.No, interfaceNo)
        # 1.可能感兴趣的人，2.可能感兴趣的组织
        self.type = get_excel("type", self.No, interfaceNo)

        self.data = {
            "click_num":self.clicknum,
            "type":self.type,
            "system": "5",
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否推荐精选动态换一换"))
