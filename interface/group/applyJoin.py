...
class 申请加入部落(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, gid, msg, uid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.gid = str(gid)
        self.msg = str(msg)
    ...
    def test_body(self):
        ...
        # 描述
        self.msg = get_excel("msg", self.No, interfaceNo)
        # 用户id
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否申请加入部落成功"))
