...
import datetime
interfaceNo = "getInterestTribeList"
name = "获取我关注的部落信息"
...
class 获取我关注的部落信息(unittest.TestCase):
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取我关注的部落信息成功"))
    ...
    def wr_excel(self):
        ...
        if self.retcode==0:
            if len(self.response["data"])>0:
                self.tribeid = self.response["data"][0]["id"]
                self.name = self.response["data"][0]["name"]
                set_excel(self.tribeid, "tribeid", self.No, interfaceNo)
                set_excel(self.name, "name", self.No, "updateTribeInfo")
