...
class 首页类目(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, needreset, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.needreset = str(needreset)
    ...
    def test_body(self):
        ...
        # 1:重置排序；0:默认用户排序
        self.needreset = get_excel("needreset", self.No, interfaceNo)
        self.data = {
            "need_reset" : self.needreset,
            "v": "3.11.0",
            "system": "5",
