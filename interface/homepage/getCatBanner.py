...
class 文章类目轮播(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, ctype, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def test_body(self):
        ...
        # 1.咨询，2.人物，3.评论
        self.ctype = get_excel("type", self.No, interfaceNo)
        self.data = {
            "type" : self.ctype,
            "v": "3.11.0",
            "system": "5",
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取文章类目轮播"))
