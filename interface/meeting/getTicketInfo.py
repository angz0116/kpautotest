...
class 获取会议门票信息(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, mid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.mid = str(mid)
    def setUp(self):
    ...
    def test_body(self):
        ...
        # 会议ID
        self.mid = get_excel("mid", self.No, interfaceNo)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取会议门票信息"))
