...
interfaceNo = "getUserArticleList"
name = "获取我的文章列表"
...
class 获取我的文章列表(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, uid, page, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.uid = str(uid)
        self.page = str(page)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取我的文章列表"))
    ...
    def wr_excel(self):
        ...
        if self.retcode==0:
            if len(self.response["data"])>0:
                ################# 存在问题，未获取到对应的id
                self.aid = self.response["data"]["list"]
                set_excel(self.aid, "aid", self.No, "getOneArticle")
