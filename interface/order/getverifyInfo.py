...
class 获取扫码会场信息(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, type, sign, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.type = str(type)
        self.sign = str(sign)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取扫码会场信息"))
