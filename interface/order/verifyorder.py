...
class 获取扫码核销(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, venueid, orderid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.type = str(type)
        self.venueid = str(venueid)
    ...
    def test_body(self):
        ...
        # 会场id
        self.venueid = get_excel("venueid", self.No, interfaceNo)
        # 订单号
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取扫码核销"))
