...
class 通知列表(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, crowdid, menuid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.crowdif = str(crowdid)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取通知列表"))
