...
class 搜索组织(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, crowdname, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.crowdname = str(crowdname)
    ...
    def test_body(self):
        ...
        # 组织名称
        self.crowdname = get_excel("crowdname", self.No, interfaceNo)
        self.data = {
            "crowd_name" :self.crowdname,
            "v": "3.11.0",
            "system": "5",
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否搜索组织成功"))
