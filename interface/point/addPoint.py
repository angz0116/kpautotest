...
class 新增积分(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, type, itemId, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.type = str(type)
    ...
    def test_body(self):
        ...
        # 所对应的item
        self.itemId = get_excel("itemId", self.No, interfaceNo)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否新增积分成功"))
