...
class 获取自动提示信息(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, keyword, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def test_body(self):
        ...
        # 搜索关键词
        self.keyword = get_excel("keyword", self.No, interfaceNo)
        self.data = {
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取自动提示信息"))
