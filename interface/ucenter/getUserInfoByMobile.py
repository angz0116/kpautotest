...
import datetime
interfaceNo = "getUserInfoByMobile"
name = "判断手机号是否注册账号"
...
class 判断手机号是否注册账号(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, mobile, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否判断手机号是否注册账号成功"))
