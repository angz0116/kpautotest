...
class 获取当前用户已认证学会列表(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, cpage, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取当前用户已认证学会列表"))
