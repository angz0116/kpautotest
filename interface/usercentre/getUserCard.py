...
class 获取用户名片二维码(unittest.TestCase):
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取用户名片二维码"))
            if self.retcode==0:
                self.cardurl = self.response["data"]["card_url"]
