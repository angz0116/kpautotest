...
class 获取签到奖励规则(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, devicetype, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def test_body(self):
        ...
        self.devicetype = get_excel("devicetype", self.No, interfaceNo)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取签到奖励规则"))
