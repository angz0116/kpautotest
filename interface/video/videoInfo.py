...
class 获取视频详情(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, vid, 预期结果):
        self.No = str(No)
        self.url = str(url)
        self.vid = str(vid)
    ...
    def test_body(self):
        ...
        # 获取视频id
        self.vid = get_excel("vid", self.No, interfaceNo)
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否获取视频详情"))
