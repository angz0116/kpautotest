...
class 用户提现接口(unittest.TestCase):
    def setParameters(self, No, 测试结果, 请求报文, 返回报文, 测试用例, url, price, 预期结果):
        self.No = str(No)
        self.url = str(url)
    ...
    def test_body(self):
        ...
        # 提现金额
        self.price = get_excel("price", self.No, interfaceNo)
        # 获取登录sheet页中token
    ...
    def check_result(self):
        try:
            self.assertEqual(self.retcode, 0, self.logger.info("是否用户提现接口"))
